...
INTENT_CATEGORIES = {
    "summarize": "The user wants to condense, shorten, or get an overview of text or a topic.",
    "infer": "The user wants sentiment analysis, topic extraction, entity recognition, or classification.",
    "transform": "The user wants translation, reformatting, converting to JSON/CSV, or changing writing style.",
    "expand": "The user wants to generate content from notes — emails, essays, blog posts, reports.",
    "general": "A general question or request that does not fit the above categories.",
}

ROUTE_SYSTEM_PROMPTS = {
    "summarize": (
        "You are ContextFlow, an expert summarizer. "
        "Your task is to produce a clear, concise summary that preserves the key points. "
    ...
    "infer": (
        ...
        "Extract the information the user asks for — sentiment, topics, entities, or categories — "
        "and return it in a structured format with short explanations for each inference. "
...
OUTPUT_EVAL_PROMPT = (
    ...
    "Given the user's question and the AI's draft answer, evaluate whether the answer is: "
...
def get_cross_encoder():
    ...
    if _cross_encoder_model is None:
        try:
            ...
            print(f"WARNING: Cross-encoder unavailable ({e}). Reranking skipped.")
...
def classify_intent(question: str) -> str:
    ...
    classification_prompt = (
        f"Classify the user's request into exactly one of these categories:\n"
...
def answer_without_pdf(question: str, chat_history: list) -> str:
    ...
    if moderation["flagged"]:
        ...
        return (
            f"I'm sorry, but I can't respond to that request. "
    ...
    if not eval_result.get("pass", True):
        fallback_messages = [
            ...
            HumanMessage(
                content=(
                    f"The following draft answer was flagged as insufficient:\n\n"
...
class AdapteredEmbeddings(Embeddings):
    ...
    def _apply_matrix(self, vec):
        ...
        p = np.dot(self.adapter_matrix, v)
        return (p / (np.linalg.norm(p) + 1e-8)).tolist()
...
def setup_vectorstore(file_path: str, adapter_matrix=None):
    ...
    final_embeddings = (
        ...
        if adapter_matrix is not None else base_embeddings
    )
...
def get_advanced_chain(base_retriever, model_name="gpt-4o-mini", top_n=3):
    ...
    if cross_encoder is not None:
        try:
            ...
            print(f"WARNING: Reranker failed ({e}). Using multi-query only.")
...
def train_embedding_adaptor(dataset, embedding_dim=1536, epochs=100, lr=0.001):
    ...
    for epoch in range(epochs):
        ...
        for query_emb, doc_emb, relevance_score in dataset:
            ...
            uq = torch.nn.functional.normalize(torch.matmul(adapter_matrix, q), dim=0)
            d = torch.nn.functional.normalize(d, dim=0)
            sim = torch.nn.functional.cosine_similarity(uq.unsqueeze(0), d.unsqueeze(0))
